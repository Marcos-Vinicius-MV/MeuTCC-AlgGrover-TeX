\section{Circuito quântico virtual}
\label{sec:circuitoVirtual}

Um circuito quântico virtual refere‑se à representação lógica e idealizada de um algoritmo quântico, descrita em termos de \textit{qubits} e portas lógicas abstratas, independente das restrições físicas ou imperfeições de qualquer hardware específico. Esta abstração permite projetar e testar algoritmos em um ambiente ideal antes de considerar as limitações práticas da implementação em \textit{hardware} real, onde aspectos como conectividade entre \textit{qubits}, fidelidade de portas e decoerência precisam ser levados em conta.
...
\begin{align}
    ...
    \ket{s} =&~ \frac{1}{4} \sum_{x=0}^{15} \ket{bin(x)} \\
     =&~ \frac{1}{4} \bigl(\ket{0000} +  \ket{0001} +  \ket{0010} +  \ket{0011} +  \ket{0100} +  \ket{0101} +  \ket{0110} + \ket{0111} + \notag \\
...
\begin{align}
    \notag
    U_f\ket{s} = ~& \frac{1}{4}~U_f\bigl(\ket{0000} + \ket{0001} + \ket{0010} + \ket{0011} + \ket{0100} + \ket{0101} + \ket{0110} +\ket{0111} + \\ \notag
    &~ \ket{1000} + \ket{1001} + \ket{1010} + \ket{1011} + \ket{1100} + \ket{1101} + \ket{1110} + \ket{1111}\bigr) \\
...
{
Como o \emph{Qiskit} não oferece suporte nativo à porta $MCZ$, ela é construída combinando portas $Hadamard$ e $MCX$, visto que $MCZ=H~MCX~H$ ($H$'s apenas no qubit alvo). A demonstração é feita no Apêndice~\ref{ap:apendiceC}.
}
