...
A Preparaç\~{a}o Inicial \'{e} a etapa inicial do algoritmo, na qual o Espaço de Pesquisa \'{e} criado, matematicamente descrito como uma superposiç\~{a}o uniforme dada pela Equaç\~{a}o~\ref{eq:superposicao}, e possui tamanho dado por $N = 2^n$, com $n$ sendo o n\'{u}mero de qubits.
...
De modo geral, a construção dos Oráculos no Algoritmo de Grover pode ser realizada utilizando portas lógicas qu\^{a}nticas do tipo $X$ e \simboloinline{$MCZ$}{\textit{Porta Lógica Quântica Z-multi-controlada}} (definida matematicamente pela matriz da Equação~\ref{mtx: gateMCZ} e em termos de portas lógicas quânticas pela Figura~\ref{fig:aplicacaoOraculo}), seguindo a seguinte regra de formação:
...
Como esta é uma reflexão sobre $\ket{s}$, busca-se adicionar uma fase negativa a cada estado ortogonal a $\ket{s}$. A maneira como pode-se fazer isso é aplicar uma transformaç\~{a}o que leva o estado $\ket{s} \to \ket{0}$, com isso, em vez de $U_s$, chamar-se-á $U_0$. Dessa forma, a Equaç\~{a}o~\ref{eq:Us} se torna: 
%
\begin{equation}
    U_0 = 2~\ket{0}\bra{0} - I.
    \label{eq:Uo}
...
Essa operação aplica uma fase negativa a todos os estados ortogonais a $\ket{0}$, como será demonstrado.
...
\begin{equation}
U_0~\ket{s} = 2~\ket{0}\bra{0} \left( \frac{1}{\sqrt{N}}\sum_{x=0}^{N-1} \ket{bin(x)}\right) - \frac{1}{\sqrt{N}}\sum_{x=0}^{N-1} \ket{bin(x)}
...
\begin{align}
\notag
U_0~\ket{s} = &\frac{2}{\sqrt{N}}~\ket{bin(0)} - \frac{1}{\sqrt{N}}\sum_{x=0}^{N-1} \ket{bin(x)} \\ 
\notag
= &\frac{2}{\sqrt{N}}~\ket{bin(0)} - \frac{1}{\sqrt{N}} \bigl(\ket{bin(0)} + \ket{bin(1)} + \ket{bin(2)} + \dots + \ket{bin(N-1)}\bigr)\\ 
= &\frac{1}{\sqrt{N}} \ket{bin(0)} - \frac{1}{\sqrt{N}} \sum_{x=1}^{N-1} \ket{bin(x)}.
...
\label{fig:difusor} 
\scalebox{1}{\begin{quantikz}%[scale=0.9, transform shape]
...
Nesse ponto, deve-se gerar a transformação que retorna de $\ket{0} \to \ket{s}$ descrita pela Equação~\ref{gate: operadorDifusao}. Tem-se,então, que o Operador de Difusão completo em portas lógicas qu\^{a}nticas são dados pela Equação~\ref{gate: operadorDifusaoCompleto} e pela Figura~\ref{fig:operadorDifusaoCompleto}.
